    ...
    <Box bg="black" width="100%" color="white">
      ...
      <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6} my={8} px={5}>
        ...
        <Box bg="#1D1D1D" p={6} borderRadius="lg" shadow="lg" textAlign="center">
          ...
          <Text fontSize="sm" color="gray.400">Access private investment documents and reports</Text>
          <Text mt={2} fontSize="xs" color={ ndaStatus !=='Approved' ? "yellow.400" : 'green.400'}>
            <Icon as={ ndaStatus !== 'Approved' ? InfoIcon : CheckCircleIcon} /> {ndaStatus !== 'Approved' ? 'NDA required for access' : 'NDA approved.'}
          </Text>
          <Button w="full" colorScheme="teal" mt={4}>View Public Documents</Button>
          <Button
            w="full"
            mt={2}
        ...
        {/* NDA Process Section */}
        <Box bg="#1D1D1D" p={6} display={'flex'} flexDirection={'column'} justifyContent={'space-between'} alignItems={'center'} borderRadius="lg" shadow="lg" textAlign="center">
        <Heading size="md" display="flex" flexDirection={'row'} gap={{md:'0.5rem',base:'0.3rem'}} alignItems="center" justifyContent="center">
        <Icon as={FaUserShield} w={8} h={8} color="grey" /> 
          NDA Process
          </Heading>
          <Badge w={'xxs'} colorScheme={ndaStatus === 'Approved' ? 'green' : 'orange'} mb={2} p={1} borderRadius="full">
            {ndaStatus}
            </Badge>
          <Text fontSize="sm" color="gray.400">Complete NDA to access sensitive documents</Text>
          <Button w="full" colorScheme={ndaStatus === 'Approved' ? 'teal':"blue"} mt={4} onClick={handleStartNdaProcess}>
        ...
        KYC Verification
          ...
          <Text fontSize="sm" color="gray.400">Know Your Customer verification process</Text>
      ...
      {/* Modals */}
      {showNdaModal && <NDARequestModal isOpen={showNdaModal} onClose={() => setShowNdaModal(false)} session={session} />}
      {showNdaDocModal && ndaMetadata && <NDADocumentModal isOpen={showNdaDocModal} onClose={() => setShowNdaDocModal(false)} session={session} />}
    </Box>
  );
};
